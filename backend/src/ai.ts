import { ExtractedOrderData, ExtractedProductItem } from './types';
...
/**
 * Extract order using Gemini REST API (Workers-compatible)
 */
          ...
          contents: [
            {
              ...
              parts: [
                {
                  text: `Extract order details from this message:\n\n"""\n${text}\n"""`,
                },
              ],
            },
          ],
 ...
 */
export async function processCustomerOrderText(
  rawText: string,
  db: Db,
  env: Env
  ...
  const openRouterConfig = await db.getOpenRouterConfig();
  ...
  if (env.GEMINI_API_KEY) {
    ...
    const geminiResult = await extractWithGemini(rawText, env.GEMINI_API_KEY);
    if (geminiResult) {
      return geminiResult;
    }
